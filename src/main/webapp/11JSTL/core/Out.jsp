...
<body>
	...
	<h4>escapeXml 속성</h4>
	<!-- escapeXml = false가 되면 HTML태그가 적용되어 출력된다. innerHTML()과 동일 -->
	<c:out value="${iTag }" escapeXml="false" />
	
	<h4>default 속성</h4>
	<!-- 최초 실행시에는 파라미터가 없는 상태이므로  default 값이 출력됨. 그러나 쿼리스트링으로 name을 써주면 그게 출력된다. -->
	<c:out value="${param.name }" default="이름 없음"/>
	<!-- value속성이 null일때만 default값이 출력되고, 빈 값인 경우에는 출력되지 않는다. -->
	<c:out value="" default="빈 문자열도 값입니다." /> <!-- 빈 문자열도 값이기 때문에, null이 아니라서 default값은 출력되지 않음 -->
</body>
</html>