		...
		*/
		...
		$.post( //넘어오는 데이터가 text로 넘어오기 때문에
			...
			function(resData) {
				...
				*/
				var d = JSON.parse(resData); //JSON이면 JSON으로 parsing해야 한다. 
				if(d.result==1){ //로그인에 성공한 경우...
					console.log(d.message);
					//콜백받은 HTML태그를 해당 위치에 삽입한다.
					$('#loginFrm').html(d.html);
					//로그인 버튼은 숨김처리 한다. (굳이 여기서는 안해도 되는데 기본적으로 로그인이 되면 로그인 버튼이 숨겨지고 로그아웃 버튼이 나와야하니까)
					$('#btnLogin2').hide();
					alert(d.message);
				}
				else{
		...
		<form id="loginFrm" onsubmit="return checkFrm();">
			<table class="table table-bordered">
				<tr>
					<td>아이디</td>
					<td>
						<input type="text" name="user_id" value="" /> <!-- form태그에서 name태그 없으면 결과가 날라가지 않음 -->
					</td>
				</tr>
				<tr>
					<td>패스워드</td>
