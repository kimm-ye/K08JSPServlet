...
<head>
    ...
    <script>
    //로그인은 보통 post방식을 사용한다. 여긴 확인을 위해 get방식 사용. 
    //매개변수를 통해 form태그의 DOM 객체를 사용할 수 있다.
    var isValidate = function(frm){
        //아이디 : 입력값이 있는지 검증. 빈값인 경우 경고창 띠움
        if(frm.id.value ==''){
            alert('아이디를 입력하세요');
            frm.id.focus();
            return false; //빈칸일때는 서버에 전송하지 않으므로
        }
        if(!frm.pwd.value || !frm.pwd2.value){
            alert('패스워드를 입력하세요');
            return false; //둘중에 하나라도 공란이면 서버전송x
        }
        ...
        입력값에 대한 검증이 아니라 선택 여부에 대한 검증을 해야 한다.
        */
        var isGender = false;
        ...
        if(isGender != true){
            alert('성별을 선택해주세요');
            //라디오는 여러 항목이 있으므로 focus()를 사용할때 인덱스를 써야 한다.
            frm.gender[0].focus();
            return false;
        }

        /*
        checkbox는 여러개 선택할 수 있으므로 체크된 항목을 카운트하여
        검증한다.
        */
        var checkCount =0;
    ...
    //아이디 중복확인
    function idCheck(fn){
        //아이디 중복확인 눌렀는데 아이디가 입력되지 않은 상태라면
        if(fn.id.value==""){
            ...
            */
            fn.id.readOnly = true; //검증이 끝난후 수정을 할 수 없게하고 팝업창에서는 수정할 수 있도록 readOnly사용 
            /*
            부모창에서 입력된 아이디를 파라미터로 팝업창으로 전달한다.
            이때 파라미터 명은 id이고 전달되는 값은 입력된 아이디이다.
            */
            window.open("./id_overapping.jsp?id=" + fn.id.value, "idover", "width=600, height=300");
        }
    }
    </script>
</head>
